...
spn_hh <- 
  ...
  dplyr::mutate(hiv = factor(if_else(hiv==0, 'hiv-',
                ...
                agecat = factor(if_else(is.na(age), NA_character_,
                                        if_else(age <=5, 'child', 'adult'))), #later make it 0-5, 6-17, 18+
                agecatx = factor(if_else(is.na(age), NA_character_,
                                        if_else(age <=5, 'ychild', 
                                                if_else(age > 5 & age <= 17, 'ochild', 'adult')))), #later make it 0-5, 6-17, 18+
...
spn_hhB <-
  dplyr::left_join(
    spn_hh %>%
      dplyr::select(hhid, pid, vno, pns, stg, age, agecat, agecatx, sex, hiv, dens, denscat) %>%
      dplyr::filter(vno == 1),
...
spn_hhF <- 
  dplyr::left_join(
    spn_hh %>%
      dplyr::select(hhid, pid, vno, pns, stg, agecat, agecatx, sex, hiv, dens, denscat),
